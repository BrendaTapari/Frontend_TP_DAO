...
import { paySancion } from "../../services/sancionesService";
import { CreditCard, Banknote, Wallet, CheckCircle } from "lucide-react";

interface SancionPaymentModalProps {
...
export default function SancionPaymentModal({ sancionId, isOpen, onClose, onSuccess }: SancionPaymentModalProps) {
  const [selectedMethod, setSelectedMethod] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState("");

  if (!isOpen || !sancionId) return null;
  ...
  return (
    <dialog className="modal modal-open">
      <div className="modal-box">
        <h3 className="font-bold text-lg mb-6">Pagar Sanción #{sancionId}</h3>
